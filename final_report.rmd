...
dataset_basic = chs_14_16 %>% 
  dplyr::select(agegroup,insure,smoker,everyday,numberperdaya,cost20cigarettes,generalhealth,imputed_povertygroup,bmi,child,sex,insuredgateway) %>% 
...
Overall, smokers' insurance status are substantially influenced by smoking which is consistent with what we expected. A research from [Jennifer Tolbert (2020)](https://www.kff.org/uninsured/issue-brief/key-facts-about-the-uninsured-population/) have found that people without health insurance have less access to healthcare due to higher cost. Higher insurance premium for smokers may deprive them of the right of medical care. Therefore, policies in tobacco surcharge should be imposed more prudently and wisely to reduce the healthcare inequality in smokers, and at the same time advocate smoking cessation.
