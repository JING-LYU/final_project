...
uninsure = analyse_smoke_insurancetype %>% 
  mutate(insure = ifelse(insure == "Uninsured", "Uninsured", "Insure"))
...
analyse_children_insurancetype = dataset_basic %>%
  drop_na(child,insure) %>% 
  group_by(child,insure) %>% 
  summarize(n = n()) %>% 
  mutate(insure = fct_reorder(insure,n)
...
analyse_children_insurancetype$sum = rep(analyse_children_sum, each = 7)
